...
export default function EditCustomerPaymentPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [cusRes, accRes, payRes] = await Promise.all([
          customersApi.getAll({ limit: 200 }),
          accountsApi.getAll({ limit: 200 }),
          customerPaymentsApi.getById(id),
        ]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await customerPaymentsApi.update(id, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-(--color-text-primary)">Edit Customer Payment</h1>
