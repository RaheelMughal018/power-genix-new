...
import type { CustomerPaymentRecord } from './page';
...
export function CustomerPaymentsTab({ payments }: Props) {
  ...
  if (payments.length === 0) {
    return <NoContentCard title="No payments received" description="No customer payments recorded for this account." />;
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-(--color-bg-secondary) border-b border-(--color-border)">
              ...
              <th className="text-left px-4 py-3 font-semibold text-(--color-text-secondary)">Customer</th>
          ...
          <tbody>
            {paginated.map((p) => (
              <tr
                ...
                onClick={() => router.push(`${ROUTES.CUSTOMER_PAYMENTS}/${p.id}`)}
              >
                ...
                <td className="px-4 py-3 text-(--color-text-primary)">{p.customer?.name ?? '—'}</td>
                <td className="px-4 py-3 text-right font-medium text-(--color-success-600)">{formatPKR(p.amount)}</td>
