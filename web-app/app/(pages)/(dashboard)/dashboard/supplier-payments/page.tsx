...
export default function SupplierPaymentsPage() {
  ...
  const {
    ...
    setPage, setSearch, setSupplierId, setAccountId, setFromDate, setToDate,
    handleExportCsv,
  } = useSupplierPayments();
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" onClick={() => router.push(`${ROUTES.SUPPLIER_PAYMENTS}/${row.id}`)}>View</Button>
          <Button size="sm" variant="outline" onClick={() => router.push(`${ROUTES.SUPPLIER_PAYMENTS}/${row.id}/edit`)}>Edit</Button>
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-(--color-text-primary)">Supplier Payments</h1>
          <p className="text-(--color-text-secondary)">Manage payments made to suppliers</p>
        </div>
      </div>
